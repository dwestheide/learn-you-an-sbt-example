...
object ReleaseNotes {
  ...
  case class SemVersion(major: Int, minor: Int, patch: Int) {
    def canonicalString = s"$major.$minor.$patch" 
  }
  ...
  object Defaults {
    ...
    def generateReleaseNotesTask = releaseNotesGenerate := {
      val (release, prevRelease) = Implementation.releaseParser.parsed
      Implementation.releaseNotes(prevRelease, release)
    }
    def dumpReleaseNotesTask = releaseNotesDump := {
      val releaseNotes = releaseNotesGenerate.evaluated
   ...
   def fileName = releaseNotesFileName := {
      release => s"${normalizedName.value}-${release.version.canonicalString}.md"
    }
   def template = releaseNotesTemplate := {
      releaseNotes =>
        ...
        s"""
         |#RELEASE NOTES for ${name.value} ${releaseNotes.release.version.canonicalString}
         |
